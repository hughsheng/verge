...
//    search_state_tv.setTypeface(typeface);
	...
	private void sendWeather() {
		addData(DataProtocolUtil.getData(DataProtocolUtil.MSG_TYPE.MSG_WEATHER, ConstanceValue.WEATHER));
	}
	...
	private void sendBattery() {
		...
		addData(DataProtocolUtil.getData(DataProtocolUtil.MSG_TYPE.MSG_BATTERY,
				battery));
//		Toast.makeText(getContext(), "发送电量状态：" + state+"tmp="+tmp+"电池电量为："+ ConstanceValue.CURRENT_BATTERY_PERCENT, Toast.LENGTH_LONG).show();
	}
